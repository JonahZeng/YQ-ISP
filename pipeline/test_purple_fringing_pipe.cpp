...
void test_purple_fringing_pipe(pipeline_manager* manager)
{
    ...
    manager->register_module(yuv2rgb_hw);
    manager->register_module(cac_rgb_hw);
    manager->register_module(rgb2yuv_hw);
    manager->register_module(yuv422_conv_hw);
    manager->register_module(yuv_encode_hw);

    yuv_decode_hw->connect_port(0, yuv2rgb_hw, 0); //y
    yuv_decode_hw->connect_port(1, yuv2rgb_hw, 1); //u
    yuv_decode_hw->connect_port(2, yuv2rgb_hw, 2); //v

    yuv2rgb_hw->connect_port(0, cac_rgb_hw, 0); //r
    yuv2rgb_hw->connect_port(1, cac_rgb_hw, 1); //g
    yuv2rgb_hw->connect_port(2, cac_rgb_hw, 2); //b

    cac_rgb_hw->connect_port(0, rgb2yuv_hw, 0); //r
    cac_rgb_hw->connect_port(1, rgb2yuv_hw, 1); //g
    cac_rgb_hw->connect_port(2, rgb2yuv_hw, 2); //b
