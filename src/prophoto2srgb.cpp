    ...
    for (uint32_t row = 0; row < ysize; row++)
    {
        for (uint32_t col = 0; col < xsize; col++)
        {
...

            {
                ...
                total_clip = total_clip / 3;

                total_clip = (total_clip > 3072) ? 3072 : total_clip;

                if(total_clip > 0)
                {
                    h_final = h_out;
                    v_final = v_clip;
                    s_final = s_out + ((s_clip - s_out) * total_clip) / 3072;
...
void prophoto2srgb::hw_run(statistic_info_t* stat_out, uint32_t frame_cnt)
{
    ...
    if (xmlConfigValid)
    {
        ...
        prophoto2srgb_reg->hlr_en = hlr_en;
    }
