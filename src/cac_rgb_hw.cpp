...
cac_rgb_hw::cac_rgb_hw(uint32_t inpins, uint32_t outpins, const char* inst_name):hw_base(inpins, outpins, inst_name)
{
    bypass = 0;
    cac_rgb_reg = new cac_rgb_reg_t;
}

void cac_rgb_hw::hw_run(statistic_info_t* stat_out, uint32_t frame_cnt)
{
...
cac_rgb_hw::~cac_rgb_hw()
{
    log_info("%s module deinit start\n", __FUNCTION__);
    if (cac_rgb_reg != nullptr)
    {
        delete cac_rgb_reg;
    }
...
void cac_rgb_hw::checkparameters(cac_rgb_reg_t* reg)
{
